...
public class UserService {
  ...
  /**
   * 1.插入数据库，用户还是非激活;因此要密码加盐md5;保存头像文件到本地
   * 2.生成key，绑定email
   * 3.发送邮件给用户 进行激活
   *
   * @param account
   * @return
   */
  @Transactional(rollbackFor = Exception.class)//事务注解，这个注解事务在另外一个类调用才会生效，在UserService另一个方法中调不生效，spring操作事务会进行AOP拦截，因为会对字节码修饰
  public boolean addAccount(User account) {
    ...
    BeanHelper.onInsert(account);//把创建时间添加进去
    account.setEnable(0);
  ...
  public List<User> getUserByQuery(User user) {
    ...
    list.forEach(u -> {  //对用户头像进行处理  在头像路径加上服务武器头路径
